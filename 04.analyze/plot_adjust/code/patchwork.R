...
read_plot <- function(city_name_t){
  ...
  int_year <- tt %>% 
    dplyr::filter(real_y == 1)
  
  int_year <- unique(int_year$time_unit)
  
  output_plot <- ggplot(tt, aes(time_unit)) +
    geom_line(aes(y = real_y)) +
    geom_line(aes(y = synth_y), linetype="dashed", size = 0.5) +
    geom_point(aes(x = time_unit, y = real_y), size = 0.8)+
    geom_vline(xintercept = int_year, linetype = "dotted", size = 0.8) +
    labs(title = title_id,
         y = "population") +
...
ggsave(first_group, filename = here::here('04.analyze','synthetic_control', 'figure',
                                          'synth_cov', 'density_1000', 'first.png'),
